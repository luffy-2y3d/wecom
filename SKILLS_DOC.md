...
```json
{
  "init_content": [
    "2024 年度销售报告",                    // 标题（自动加粗）
    {"type": "text", "content": "摘要"},     // 正文段落 1
    {"type": "image", "url": "https://..."}, // 图片
    {"type": "text", "content": "详细数据"}  // 正文段落 2
  ]
}
```

**注意事项**：
- ✅ 每个文本元素会自动添加到新段落（自动添加换行符）
- ✅ 每次插入都会重新获取最新文档索引，确保顺序正确
- ✅ 图片会自动上传并插入到对应位置
- ❌ 不要在 init_content 中使用复杂的格式控制（如手动换行符）
